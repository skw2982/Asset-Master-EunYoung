...
export default function MomAssetMaster() {
  ...
  const [rateStale,    setRateStale]    = useState(false);   // 환율 실패 표시
  ...
  const [propertyGoal, setPropertyGoal] = useState({
    name: "여의도 미성 47평",
    price: 2800000000,
  });
  ...
  // ── 데이터 로드 (일반 async 함수 — useCallback 제거로 무한루프 차단) ──
      ...
      setStocks(
        sRows
          .map((row) => {
            ...
            return {
              market:      c[0], account: c[1], name: c[2],
              avg:         cleanNum(c[5]),
      ...
      setSavings(
        svRows.map((row, i) => {
          ...
          return {
            id:           i,
            name:         c[0],
            monthly:      cleanNum(c[1]),
            current:      cleanNum(c[2]),
            maturityDate: c[3],
            transferDay:  cleanNum(c[4]),
            interestRate: cleanNum(c[5]),
          };
        })
      );
  ...
  const saveGoal = async () => {
    ...
    try {
      const next = {
        name:  goalDraft.name,
        price: cleanNum(goalDraft.price),
      };
  ...
  // ── 초기 1회 로드 — 의존성 배열 [] 로 무한루프 차단 ──
  useEffect(() => {
    setIsClient(true);

    const loadGoalData = async () => {
      try {
        const res = await fetch(`${KV_URL}/get/mom_property_goal`, {
  ...
  const totals = useMemo(() => {
    const stock  = stocks.reduce(
      (a, b) => a + b.current * (b.market.includes("해외") ? exchangeRate : 1) * b.qty, 0
    );
      ...
      <div className="max-w-5xl mx-auto relative">
        ...
        <header className="mb-8 flex flex-wrap justify-between items-end border-b border-slate-800 pb-6 gap-4">
          <div>
            ...
            <p className="text-slate-500 text-[10px] font-bold tracking-[0.3em] uppercase mt-1">
              ...
              {rateStale && (
                <span className="ml-2 text-amber-500">⚠ 환율 업데이트 실패 (1,350 고정)</span>
              )}
            </p>
          </div>
          <button
          ...
          <div className="space-y-6 animate-in fade-in duration-500">
            <div className="bg-gradient-to-br from-rose-900/40 via-slate-900 to-slate-900 p-8 md:p-12 rounded-[40px] border border-rose-500/20 shadow-2xl">
              <div className="flex flex-wrap justify-between items-start gap-8">
                <div>
                  <p className="text-rose-400 text-[10px] font-black uppercase mb-3 opacity-80 tracking-widest">
                    🏡 Current Net Worth
                  </p>
                      ...
                      return (
                        ...
                        <tr key={`${s.market}-${s.name}`} className="hover:bg-white/[0.02]">
                          <td className="px-6 py-4 font-bold text-slate-200">
                            <span className="text-[10px] text-rose-400 mr-2 uppercase">
                              {isOS ? "US" : "KR"}
                            </span>
                            {s.name}
                            <div className="text-[10px] text-slate-600 mt-1">
                              {s.qty}주 · 평단 {isOS ? `$${s.avg}` : fmt(s.avg)}
                            </div>
                          </td>
                          <td className="px-6 py-4 text-right">
                            ...
                            <div className="text-[10px] opacity-60">
                              수익률 {((s.current / s.avg - 1) * 100).toFixed(1)}%
                            </div>
                          </td>
                        </tr>
                      );
        ...
        {activeTab === "savings" && (
          <div className="space-y-6 animate-in fade-in duration-500">
            {savings.map((s) => {
              ...
              // ✅ 수정된 이자 계산 (과대산정 버그 수정)
              // 기존 잔액에 대한 단리 이자
              const cInt = s.current * r * (mLeft / 12);
              // 미래 납입액에 대한 등차급수 이자 (current와 분리)
              ...
              return (
                <Card key={s.id} className="p-6">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div>
                      <p className="text-[10px] text-slate-500 uppercase">현재 잔액</p>
                      <p className="font-bold">{fmt(s.current)}원</p>
                    </div>
                    <div>
                      <p className="text-[10px] text-slate-500 uppercase">금리</p>
                      <p className="font-bold text-emerald-400">{s.interestRate}%</p>
                    </div>
                    <div>
                      <p className="text-[10px] text-slate-500 uppercase">예상 이자</p>
                      <p className="font-bold text-amber-400">{fmt(cInt + fInt)}원</p>
                    </div>
                    <div className="col-span-1 bg-emerald-950/40 p-4 rounded-2xl text-right">
                ...
                <div className="px-6 py-4 bg-slate-800/40 flex justify-between items-center border-b border-slate-800">
                  ...
                  <span
                    className={`font-black ${
                      realizedGrouped[m].sub >= 0 ? "text-emerald-400" : "text-rose-400"
                    }`}
                  >
            ...
            <Card className="p-8 md:p-12">
              <div className="flex items-center justify-between mb-8">
                ...
                <button
                  onClick={() => {
                    if (!goalEditing) {
                      setGoalDraft({
                        name:  propertyGoal.name,
                        price: propertyGoal.price.toString(),
                      });
                    }
                    setGoalEditing((v) => !v);
                  }}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-4">
                  <div>
                    ...
                    {goalEditing ? (
                      <input
                        ...
                        className="w-full bg-slate-800 text-white font-bold p-4 rounded-2xl border border-slate-600 outline-none focus:border-rose-500"
                      />
                    ) : (
                      <div className="w-full bg-slate-950 text-slate-300 font-bold p-4 rounded-2xl border border-slate-800">
                        {propertyGoal.name}
                      </div>
                    )}
                  </div>
                  <div>
                    ...
                    {goalEditing ? (
                      <input
                        ...
                        onChange={(e) =>
                          setGoalDraft((d) => ({ ...d, price: e.target.value }))
                        }
                        className="w-full bg-slate-800 text-white font-bold p-4 rounded-2xl border border-slate-600 outline-none focus:border-rose-500"
                      />
                    ) : (
                      <div className="w-full bg-slate-950 text-slate-300 font-bold p-4 rounded-2xl border border-slate-800">
                        {fmt(propertyGoal.price)}
                      </div>
                    )}
                  </div>
                  ...
                  {goalEditing && (
                    <button
                      onClick={saveGoal}
                      disabled={goalSaving}
                      className="w-full py-3 rounded-2xl font-black text-sm bg-rose-600 text-white hover:bg-rose-500 disabled:opacity-50 transition-all"
                    >
                      {goalSaving ? "저장 중..." : "💾 저장"}
                    </button>
                  )}
                ...
                <div className="bg-rose-500/5 border border-rose-500/20 rounded-[32px] p-8 flex flex-col justify-center">
                  <p className="text-rose-400 text-xs font-black uppercase mb-2">
                    구매까지 필요한 추가 금액
                  </p>
                  ...
                  <p className="text-slate-500 text-sm mt-3 font-mono">
                    약 {fmtShort(Math.max(0, goalGap))}원 더 모으면 {propertyGoal.name} 입성!
                  </p>
                  <div className="mt-6">
        ...
        <footer className="mt-20 py-8 border-t border-slate-900 text-center">
          <p className="text-slate-800 text-[10px] font-black tracking-widest uppercase italic">
            MOM&apos;S ASSET MASTER V1.4 · CREATED BY SON
          </p>
        </footer>
      </div>
    </div>
